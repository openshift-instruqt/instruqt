...
- id: bvwh9tmbnhpg
  ...
  type: website
  url: https://patient-portal-frontend-public.crc.${_SANDBOX_ID}.instruqt.io
  new_window: true
- id: i53yvwha94o8
  title: Network Observer
  type: website
  url: https://skupper-network-observer-public.crc.${_SANDBOX_ID}.instruqt.io
  new_window: true
difficulty: basic
timelimit: 1800
...
This process will install Service Interconnect in the **public** namespace by creating a **site** on the OpenShift cluster.
> [!NOTE]
...
- To initialize Service Interconnect routers in the OpenShift namespace namespace you will need to create a site by issuing the following command in the **Terminal-OpenShift**  window:
	```
	skupper site create openshift --enable-link-access
	```
> **Note**
> *You might see some warnings about Pod Security violations in the console output. These can safely be ignored.*
- Output
	```,nocopy
	Site "openshift" is not yet ready: containers with unready status: [kube-adaptor]
	```
> **Note**
> *The above message indicates that the router installation has started and will be ready soon*
...
- Output
	```,nocopy
	File written to /home/user1/.local/share/skupper/namespaces/default/input/resources/sites/rhel.yaml
	```
...
- Generate the token in the **Terminal-OpenShift** tab
	```
	skupper token issue -e 1h my-token.yaml
	```

> [!NOTE]
>  In our example we are extending the validity of our to an hour using the expiration window flag (-e)
...
- View the token
	...
	cat my-token.yaml
	```
- Output ![token.png](../assets/Screenshot%202025-05-01%20at%2011.57.32%E2%80%AFAM.png))
...
- Move the token to the resources folder
```
cp db_token.yaml /home/user1/.local/share/skupper/namespaces/default/input/resources/db_token.yaml
```
...
- Run the following command to deploy the **Service Interconnect Network Console**
```
oc apply -f https://raw.githubusercontent.com/app-connectivity-workshop/scripts/refs/heads/main/m1/network_console_deploy.yaml -n public
```
...
- Start the Virtual Application Network by running the command below from the **Terminal-RHEL**.
```
...
- You can also click on the **Network-Observer** tab  in order to visualize the network using a graphical user interface. ![network-observer.png](../assets/Screenshot%202025-05-01%20at%201.25.19%E2%80%AFPM.png)

- Click on the **Login with OpenShift** button ![openshift-login.png](../assets/Screenshot%202025-05-01%20at%201.32.01%E2%80%AFPM.png)

- Use the credentials username `admin` and password `admin` to login ![credentials.png](../assets/Screenshot%202025-05-01%20at%201.32.19%E2%80%AFPM.png)

- Allow Selected Permissions ![allow-permissions.png](../assets/Screenshot%202025-05-01%20at%201.32.29%E2%80%AFPM.png)

- You should be able to visualize the Virtual Application Network between Openshift and RHEl machine ![topology.png](../assets/Screenshot%202025-05-01%20at%202.24.22%E2%80%AFPM.png)
